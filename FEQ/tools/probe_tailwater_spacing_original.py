#!/usr/bin/env python3
"""Capture the unchanged FEQUTL 5.80 QVSTW tailwater spacing loop and square-root stores.
...
def fixtures():
    rng=random.Random(5801061697);cases=[]
    for scale in (.0001,1.,10000.):
        for index in range(1024):
            low=real(rng.uniform(-100,100)*scale)
            high=real(low+rng.uniform(.0001,100)*scale)
            if high<=low:continue
            count=rng.choice((3,4,5,7,10,21,51,101,149))
            point=rng.randrange(2,count)
            power=real(rng.choice((.25,.5,1.,1.3333333,1.5,2.,2.3333333,3.,4.,7.,8.)))
            cases.append(dict(name=f'scale-{scale}-case-{index}',
                fields=[low,high,power],count=count,index=point))
    return cases
...
def image(original,pe,symbols,count):
    ...
    code.emit('a1'+struct.pack('<I',record).hex()+'a3'+struct.pack('<I',0xf2f4e4).hex())
    code.emit('5355bd'+struct.pack('<I',scratch+4096).hex())
    code.emit('b8'+struct.pack('<I',record+4).hex())
    # Original SQRTDP(1) sequence. Its two cached input values remain below
    # the stored result; discard them before entering the spacing loop.
    code.code.extend(pe.get_data(0x12ee6,0x22))
    code.emit('ddd8ddd8')
    code.emit('d905'+struct.pack('<I',record+4).hex()+'dbbd54ffffff')
    code.emit('dd05'+struct.pack('<I',0x640318).hex()+'dbbd48ffffff')
    code.emit('c74518'+struct.pack('<I',record+12).hex())
    code.emit('c7451c'+struct.pack('<I',record+8).hex())
    # The complete original loop and its _g_arxr calls execute in place.
    # Only the first instruction after the loop is replaced with RET.
    code.call(base+0x12f7d)
    code.emit('a1'+struct.pack('<I',record+16).hex())
    code.emit('8b1485bc0064008915'+struct.pack('<I',scratch+32).hex())
    code.emit('b9'+struct.pack('<I',record+4).hex())
    code.emit('d905'+struct.pack('<I',scratch+32).hex())
    code.code.extend(pe.get_data(0x14007,0x1a))
    code.emit('dd05'+struct.pack('<I',0x640318).hex()+'dd1d'+struct.pack('<I',scratch+36).hex())
    code.emit('dd05'+struct.pack('<I',0x641128).hex()+'dd1d'+struct.pack('<I',scratch+44).hex())
    code.emit('dd04c510036400dd1d'+struct.pack('<I',scratch+52).hex())
    code.emit('dd04c520116400dd1d'+struct.pack('<I',scratch+60).hex())
    code.emit('5d5b')
    ...
    if len(driver)>later[0]-symbols['_MAIN__']['rva']:raise ValueError('Tailwater spacing driver exceeds PROGRAM body.')
    result=bytearray(original);offset=pe.get_offset_from_rva(symbols['_MAIN__']['rva']);result[offset:offset+len(driver)]=driver
    result[pe.get_offset_from_rva(0x1313c)]=0xc3
    return bytes(result)
...
def main():
    ...
    if process.returncode or len(process.stdout)!=36*len(cases):
        ...
        raise RuntimeError(f'Original tailwater spacing driver failed: exit {process.returncode}, {len(process.stdout)} bytes.')
    expected=process.stdout;(output/'outputs.bin').write_bytes(expected)
    manifest=dict(original_sha256=ORIGINAL_SHA256,routines=['_qvstw_','_g_arxr'],cases=len(cases),
        original_rva=['0x12ee6..0x12f08','0x12f7d..0x1313c','0x14007..0x14021'],
        instruction_sha256=[hashlib.sha256(pe.get_data(rva,length)).hexdigest() for rva,length in ((0x12ee6,0x22),(0x12f7d,0x1bf),(0x14007,0x1a))],
        input_record_bytes=20,output_record_bytes=36,
    ...
    if args.native:
        native=subprocess.run([str(args.native.resolve()),'--tailwater-spacing'],input=inputs,capture_output=True,timeout=30)
    ...
    print(f'{len(cases)} original tailwater levels, head drops and REAL square roots captured.',flush=True)
