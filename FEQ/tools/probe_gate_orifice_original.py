#!/usr/bin/env python3
"""Capture unchanged UFGATE upstream-head and FNDFOQ arithmetic blocks.
...
BLOCKS = ((0x80bf8,0x80c10),(0x7ee92,0x7eef4))


def fixtures():
    rng=random.Random(580106150);cases=[]
    for scale in (.0001,1.,10000.):
        for index in range(384):
            depth=real(rng.uniform(.001,100)*scale)
            datum=real(rng.uniform(-1000,1000)*scale)
            bottom=real(datum+rng.uniform(-1,1)*scale)
            opening=real(rng.uniform(.001,10)*scale)
            gate_bottom=real(datum+rng.uniform(-1,1)*scale)
            coefficient=real(rng.uniform(.4,1.2));contraction=real(rng.uniform(.3,1.))
            area=real(rng.uniform(.001,100)*scale*scale)
            effective=real((coefficient*contraction)*area)
            alpha=real(rng.uniform(1.,3.))
            # A positive free-flow radicand; ratios approach the singular
            # boundary as well as covering negligible approach velocity.
            ratio=(.000001,.01,.1,.4,.9,.999)[index%6]
            upstream_area=real(effective*(alpha**.5)/ratio)
            contraction_head=(opening*contraction+gate_bottom)-datum
            head=real(contraction_head+rng.uniform(.001,100)*scale)
            gravity=real((9.80665,32.174,32.2)[index%3])
            fields=list(map(real,(depth,bottom,datum,head,coefficient,contraction,
                area,opening,gate_bottom,upstream_area,alpha,2*gravity)))
            cases.append(dict(name=f'random-scale-{scale}-{index}',fields=fields))
    for datum in (-1048576.,-1000.,0.,450.,1000.,1048576.):
        for depth in (0.,.00001,.1,.7544864416122437,1.,2.7975,46.58898162841797):
            for coefficient in (.5,.61,1.):
                # Equal bottom and datum test cancellation, including the
                # first upstream transition head in the supplied example.
                fields=list(map(real,(depth,datum,datum,depth,coefficient,.5,
                    2.7,0.,datum,27.,1.3,64.4)))
                cases.append(dict(name=f'datum-{datum}-depth-{depth}-cd-{coefficient}',fields=fields))
    return cases
...
def image(original,pe,symbols,count):
    ...
    for index,destination in enumerate((0x5aa500,common+48,0x5aa4b0)):
    ...
    code.emit('a1'+struct.pack('<I',0x5aa49c).hex()+'a3'+struct.pack('<I',scratch+32).hex())
    for index,destination in enumerate((common+128,common+132,common+120,common+116,
                                       common+52,common+28,common+108,common+140)):
        code.emit('a1'+struct.pack('<I',record+16+index*4).hex()+'a3'+struct.pack('<I',destination).hex())
    # FNDFOQ's unchanged arithmetic expects these original register arguments
    # and a four-byte local square-root store at EBP-4.
    code.emit('53565755bd'+struct.pack('<I',frame).hex())
    code.emit('bb'+struct.pack('<I',record+12).hex()+'be'+struct.pack('<I',record+8).hex())
    code.emit('bf'+struct.pack('<I',scratch+40).hex())
    code.code.extend(pe.get_data(BLOCKS[1][0],BLOCKS[1][1]-BLOCKS[1][0]))
    code.emit('5d5f5e5b')
    code.emit('a1'+struct.pack('<I',common+76).hex()+'a3'+struct.pack('<I',scratch+36).hex())
    ...
    if len(driver)>later[0]-symbols['_MAIN__']['rva']:raise ValueError('Gate orifice driver exceeds PROGRAM body.')
...
def main():
    ...
    inputs=b''.join(struct.pack('<12f',*case['fields']) for case in cases)
    ...
    if process.returncode or len(process.stdout)!=12*len(cases):
        ...
        raise RuntimeError(f'Original gate orifice driver failed: exit {process.returncode}, {len(process.stdout)} bytes.')
    expected=process.stdout;(output/'outputs.bin').write_bytes(expected)
    manifest=dict(original_sha256=ORIGINAL_SHA256,routines=['_ufgate_','_fndfoq_'],cases=len(cases),
        ...
        input_record_bytes=48,output_record_bytes=12,
    ...
    if args.native:
        native=subprocess.run([str(args.native.resolve()),'--gate-orifice'],input=inputs,capture_output=True,timeout=30)
    ...
    print(f'{len(cases)} original upstream-head and free-orifice calculations captured.',flush=True)
