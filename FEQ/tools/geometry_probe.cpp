// Direct original-executable fixtures for section geometry.
...
std::uint32_t read_word() {
    ...
    if (!std::cin) {
        throw std::runtime_error("Incomplete section geometry fixture.");
    }
    ...
    try {
        while (std::cin.peek() != std::char_traits<char>::eof()) {
            ...
            if (point_count < 2 || point_count > 10000 || subsection_count == 0 || subsection_count > 1000) {
                throw std::runtime_error("Invalid section geometry fixture extent.");
            }
            ...
            for (std::uint32_t index = 0; index < subsection_count; ++index) { write_float(output[index].top_width); }
            for (std::uint32_t index = 0; index < subsection_count; ++index) { write_float(output[index].perimeter); }
            for (std::uint32_t index = 0; index < subsection_count; ++index) { write_double(output[index].area); }
            for (std::uint32_t index = 0; index < subsection_count; ++index) { write_double(output[index].first_moment); }
            for (std::uint32_t index = 0; index < subsection_count; ++index) { write_float(output[index].roughness_weight); }
            for (std::uint32_t index = 0; index < subsection_count; ++index) { write_float(output[index].maximum_depth); }
        }
        return std::cout ? 0 : 1;
