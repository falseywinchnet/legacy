...
extern "C" {
 ...
 static int
#ifdef KR_headers
rd_Z(n,w,len) char *n; ftnlen len;
#else
rd_Z(char *n, int w, ftnlen len)
#endif
{
	long x[9];
	char *s, *s0, *s1, *se, *t;
 ...
 static int
#ifdef KR_headers
rd_I(n,w,len, base) char *n; int w; ftnlen len; int base;
#else
rd_I(char *n, int w, ftnlen len, int base)
#endif
{
	int ch, sign;
	longint x = 0;
 ...
 have_x:
	if(len == sizeof(integer))
		feq_io_store<integer>(n, static_cast<integer>(x));
	else if(len == sizeof(char))
		feq_io_store<signed char>(n, static_cast<signed char>((char)x));
#ifdef Allow_TYQUAD
	else if (len == sizeof(longint))
		feq_io_store<longint>(n, static_cast<longint>(x));
#endif
	else
		feq_io_store<short>(n, static_cast<short>((short)x));
	if (w) {
		while(--w)
			GET(ch);
		return errno = 115;
...
{	int ch, dot, lv;
 ...
 bad:
	...
	switch(len) {
		case sizeof(char):	feq_io_store<char>(n, static_cast<char>(lv));	 break;
		case sizeof(short):	feq_io_store<short>(n, static_cast<short>(lv)); break;
		default:		feq_io_store<ftnint>(n, lv);
		}
	while(w-- > 0) {
		GET(ch);
		if (ch == ',' || ch == '\n')
 ...
 static int
#ifdef KR_headers
rd_F(p, w, d, len) char *p; ftnlen len;
#else
rd_F(char *p, int w, int d, ftnlen len)
#endif
{
...
zero:
	if (len == sizeof(real))
		feq_io_store<real>(p, static_cast<real>(x));
	else
		feq_io_store<doublereal>(p, static_cast<doublereal>(x));
	return(0);
	}
...
{	int ch;
	...
	case G:
	case GE:
	case F:	ch = rd_F(ptr,p->p1,p->p2.i[0],len);
		break;
