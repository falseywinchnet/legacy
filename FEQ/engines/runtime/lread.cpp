...
#endif
{
	...
	loopend:
		...
		case TYINT1:
		case TYLOGICAL1:
			feq_io_store<char>(ptr, static_cast<char>((char)f__lx));
			break;
		case TYLOGICAL2:
		case TYSHORT:
			feq_io_store<short>(ptr, static_cast<short>((short)f__lx));
			break;
		case TYLOGICAL:
		case TYLONG:
			feq_io_store<ftnint>(ptr, static_cast<ftnint>((ftnint)f__lx));
			break;
#ifdef Allow_TYQUAD
		case TYQUAD:
			feq_io_store<longint>(ptr, f__llx != 0 ? f__llx : static_cast<longint>(f__lx));
			break;
#endif
		case TYREAL:
			feq_io_store<real>(ptr, static_cast<real>(f__lx));
			break;
		case TYDREAL:
			feq_io_store<doublereal>(ptr, static_cast<doublereal>(f__lx));
			break;
		case TYCOMPLEX:
			feq_io_store<real>(ptr, static_cast<real>(f__lx));
			feq_io_store<real>(ptr+sizeof(real), static_cast<real>(f__ly));
			break;
		case TYDCOMPLEX:
			feq_io_store<doublereal>(ptr, f__lx);
			feq_io_store<doublereal>(ptr+sizeof(doublereal), f__ly);
			break;
		case TYCHAR:
