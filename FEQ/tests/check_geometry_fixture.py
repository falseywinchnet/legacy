#!/usr/bin/env python3
"""Require all mixed-precision geometry words captured from original FBASEL."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/section_geometry'
...
if process.returncode or process.stdout != expected:
    print(f'Section geometry mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable section geometries match every output bit.')
