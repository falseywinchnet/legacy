#!/usr/bin/env python3
"""Require every value and derivative bit captured from original LKTAB."""
from pathlib import Path
import subprocess
import sys

fixtures = Path(__file__).resolve().parent/'reference/function_tables'
...
if process.returncode or process.stdout != expected:
    print(f'Function-table mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
    ...
    for index,(left,right) in enumerate(zip(expected,process.stdout)):
        if left != right:
            ...
            break
    raise SystemExit(1)
print(f'All {len(inputs)//32} original-executable function-table fixtures match every output bit.')
