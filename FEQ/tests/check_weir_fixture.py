#!/usr/bin/env python3
"""Require all raw weir_flow outputs captured from the original executable."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/weir_flow'
...
if process.returncode or process.stdout != expected:
    print(f'weir_flow mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable weir_flow cases match every output bit.')
