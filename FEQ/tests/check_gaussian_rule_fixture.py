#!/usr/bin/env python3
"""Require all raw gaussian_rule outputs captured from the original executable."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/gaussian_rule'
...
if process.returncode or process.stdout != expected:
    print(f'gaussian_rule mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable gaussian_rule cases match every output bit.')
