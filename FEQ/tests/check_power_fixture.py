#!/usr/bin/env python3
"""Require all raw power outputs captured from the original executable."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/power'
...
if process.returncode or process.stdout != expected:
    print(f'Power mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable power cases match every output bit.')
