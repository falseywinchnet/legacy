#!/usr/bin/env python3
"""Require all raw trigonometry outputs captured from the original executable."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/trigonometry'
...
if process.returncode or process.stdout != expected:
    print(f'trigonometry mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable trigonometry cases match every output bit.')
