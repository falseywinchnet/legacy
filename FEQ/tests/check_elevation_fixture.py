#!/usr/bin/env python3
"""Require all raw elevation sequences outputs captured from the original executable."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/elevation_arguments'
...
if process.returncode or process.stdout != expected:
    print(f'Section elevation sequences mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable elevation sequences match every output bit.')
