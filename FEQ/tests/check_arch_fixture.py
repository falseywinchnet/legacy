#!/usr/bin/env python3
"""Require all raw arch_perimeter outputs captured from the original executable."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/arch_perimeter'
...
if process.returncode or process.stdout != expected:
    print(f'arch_perimeter mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable arch_perimeter cases match every output bit.')
