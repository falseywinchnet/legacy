#!/usr/bin/env python3
"""Require the original coefficient return, REAL store, and head loss."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/culvert_loss'
...
if process.returncode or process.stdout != expected:
    print(f'Culvert loss mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original culvert-loss cases match every output bit.')
