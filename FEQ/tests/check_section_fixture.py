...
fixtures = Path(__file__).resolve().parent/'reference/section_interpolation'
...
if process.returncode or process.stdout != expected:
    ...
    for index,(left,right) in enumerate(zip(expected,process.stdout)):
        if left != right:
            ...
            break
    raise SystemExit(1)
print(f'All {len(inputs)//64} original-executable section fixtures match every output bit.')
