#!/usr/bin/env python3
"""Require all raw section_sinuosity outputs captured from the original executable."""
from pathlib import Path
import subprocess
import sys
import json

fixtures = Path(__file__).resolve().parent/'reference/section_sinuosity'
...
if process.returncode or process.stdout != expected:
    print(f'section_sinuosity mismatch: exit {process.returncode}; expected {len(expected)} bytes, received {len(process.stdout)}.')
    if process.stderr:
...
print(f'All {count} original-executable section_sinuosity cases match every output bit.')
