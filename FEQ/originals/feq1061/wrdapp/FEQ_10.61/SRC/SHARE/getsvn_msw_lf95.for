...
c     Local
      ...
      integer i, it, stdun, mflag, under_svn
...
52    format(/,'*WRN:XXX* The working copy contains local ',
     ...
     d'at a later time.')
54    format(/,'*ERR:XXX Working directory is under Subversion but',
...
c***********************************************************************
      ...
      svn_rev = 0
      svn_mod = 0
      under_svn = 0 ! lf95 does not support a "system" function:(
...
c     If the home directory is under Subversion, there will be one or 
c     more directories in the subdirectory, .svn.  Otherwise, .svn 
c     will not exist and an error message will be given.  Assuming that
c     the standard msw command prompt will be used by the system command,
c     the error message should be "File Not Found".
      read(stdun,'(a)') temp
      under_svn = index(temp,'File Not Found')
      close(stdun)
      call system('del /q dir.zxcv_ > NUL 2>&1')  !delete the file-no longer needed
      if(under_svn == 0) then
c       The home directory is under subversion.  Run svnversion and 
c       get the info on the versions
        it = len_trim(home_dir)
        call system('svnversion '//home_dir//' > svn.zxcv_ 2>&1')
        stdun = get_unit(stdout)        
        open(unit=stdun, file='svn.zxcv_',status='old')
        read(stdun,'(a)') temp
c       Check if svnversion was available
        i = index(temp, 'not recognized as')
        if(i == 0) then
c         Try another command--4NT
          i = index(temp, 'Unknown command')
        endif
        if(i > 0) then
c         Looks like svnversion is not available.
c        Command-line subversion may not be installed
          write(stdout,54)
        else
          it = len_trim(temp)
...
c           There is a colon.  Read the integer after it.
          ...
          call system('del /q svn.zxcv_ > NUL 2>&1')
        endif
      endif
      if(svn_rev > 0 ) then
