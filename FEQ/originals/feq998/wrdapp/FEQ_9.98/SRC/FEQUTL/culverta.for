...
C     branch in FEQ but with fewer options
      ...
      IMPLICIT NONE
C     + + + DUMMY ARGUMENTS + + +
      INTEGER BSHAPE, CHKBAR, EFLAG, FTKNT, FTP, IAT3D, IAT6D, STDIN,
     A        STDOUT, MFTNUM, MNBN, MRFTAB, NBN, NBRA, TYPFLG
...
C               (new type 22) must be supplied.  TYPFLG = 0 when called
C               from CULVRT and TYPFLG=1 when called from from XSTMAK.
C     EFLAG  - Error flag: EFLAG=0-no errors; else one or more errors
...
C     + + + LOCAL VARIABLES + + +
     ...
     A     SBKIND, SDXOVL, STAT, STATL, STATR, STDL, STDR, X, Z,
     B     XL, XR
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL CHKTAB, FNDELV, FNDSTA, GETD, GETTOK, INL80, INL120, KIL,
...
C     + + + LOCAL VARIABLES + + +
     ...
     G     ZDATUM, ZSBRDF, ZT, Y2MAT(PMXNHU,PMXFRC)
...
C     + + + EXTERNAL NAMES + + +
     ...
     B         GETTOK, INL80, INPRDP, INSERT, LKTA, LKTAB, LKTK, QVSTW,
...
C     MAKE AS CLOSE TO A BRANCH AS POSSIBLE.
     ...
     O           LINE)
      ...
      NBRA = 1
C     TYPFLG at 1 request automatic interpolation for cross sections
...
C          CALL TWOD13GET
C     I                  (STDOUT, 
C     O                   PUTQ, NHU, NFRAC, HUVEC, FDVEC,
C     O                   PFDVEC, QMAT, HDATUM, TABTYP, 
C     O                   EFLAG)
C        CALL TWDOUT
C     I             (STDOUT, STDOUT, PUTQ, LABEL, NHU, NFRAC, HUVEC,
C     I              FDVEC, PFDVEC, QMAT, HDATUM,
C     M              TABTYP, ' CULVERT',
C     O              EFLAG)
        ENDIF
        IF(PUTY2.GT.0) THEN
