...
C     + + + OUTPUT FORMATS + + +
 52   FORMAT('0*ERR:510* Duplicate table number.')
C***********************************************************************
...
C     + + + OUTPUT FORMATS + + +
 50   FORMAT(' *WRN:520* CELERITY DECREASES BY ',F7.2,' PER CENT AT',
     A   ' DEPTH=',F10.3)
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETTOK, GRULE, SORT2, STRIP_L_BLANKS, INL80
...
C     + + + OUTPUT FORMATS + + +
 ...
 72   FORMAT(' Version: ',F6.2,' Version date: ',A17,10X,
...
C     GET THE FIRST FILE ARGUMENT
     ...
     O            FNAME)
      ...
      ELSE
        ...
        WRITE(*,*) ' Please check spelling of user-input file.'
        STOP 'Abnormal stop. Errors found.'
      ENDIF
...
C       INPUT FUNCTION TABLES FOR ACCESS BY SUBSEQUENT COMMANDS.
 ...
 300    CONTINUE
        ...
        WRITE(STDOUT,*) ' '
        WRITE(STDOUT,'('' THERE ARE NOW'',I5,
     A   '' FUNCTION TABLES STORED'')') FTKNT
        WRITE(STDOUT,'(3X, I7,'' LOCATIONS OUT OF '',I7,'' ARE USED'')')
     A   FTP, MRFTAB
        GOTO 1000
