...
C     + + + LOCAL PARAMETERS + + +
     ...
     B         NUMERIC, CHAR, N_SYMBOL, NXTBLK, ENDSIG, NONE
      PARAMETER(N_SYMBOL=16, INTVAL=1, REAVAL=2,
...
C     + + + SAVED VALUES + + +
     ...
     D 'Depth'/
      ...
      DATA GROUP  /
     A   6*CHAR, 3*NUMERIC,ENDSIG,6*NXTBLK/          

      DATA GROUP_INDEX /
     A  1, 2, 3, 4, 5, 6, 1, 2, 3, 7*0/
                                       
      DATA RESPONSE_TYPE  /
     A  6*CHRVAL,3*REAVAL,7*NONE/
    
      DATA CONVERT_RULE /
     A  4*EXACT,2*LOWER,3*LOWER,7*EXACT/
...
C     + + + LOCAL VARIABLES + + +
     ...
     A        NEXT, NTAB, LFLAG
      ...
      INTEGER ITEM_START(MAXN), ITEM_END(MAXN), ADJLOC(MNDEP)
     ...
     B       IFAC(4)
      ...
      CHARACTER*16 FIT_WITH, CHK_OPTION, LEFT_SLOPE, RIGHT_SLOPE,
     A             TABID
      CHARACTER LINE*120, JUST*5, AREA*10, VOLUME*10
...
C     + + + EXTERNAL NAMES + + +
      ...
      EXTERNAL  INL120,  KIL, LENSTR, VAR_DECIMALD,
