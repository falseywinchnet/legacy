...
C     Local variables.
      ...
      REAL TIME, DQED, DQEU, NEWHG, Q

      DATA TIME/0.0/, MFTAB/0/

C***********************************************************************
C     Find the flow at the current gate opening 

      CALL TDLK15
     I           (GATE_RES_STDOUT, GATE_RES_TABLE, MFTAB, TIME, 
...
C     from zero.
      ...
      INTEGER STDOUT, UD_TABLE, DU_TABLE, RESULT_FLAG
...
C     Local variables
      ...
      INTEGER MFTAB, FLAG, FTYPE_CODE

      REAL TIME, DQED, DQEU, NEWHG, Q, A, B, HG_RESULT
...
C     target value.  If not, no solution is possible.  
      ...
      IF(GATE_FLOW.GT.0.0) THEN
        CALL TDLK15
     I             (STDOUT, UD_TABLE, MFTAB, TIME, EDN, EUP, HG_MAX,
     I              HBASE,
     O              Q, DQED, DQEU, NEWHG, FTYPE, FTYPE_CODE)
      ELSE
        CALL TDLK15
     I             (STDOUT, DU_TABLE, MFTAB, TIME, EUP, EDN, HG_MAX,
     I              HBASE,
     O              Q, DQED, DQEU, NEWHG, FTYPE, FTYPE_CODE)
      ENDIF
...
C     MIN_PER_CELL(*,*) - minimum gate opening in a cell.
     ...
     C        N_PER_CELL(MRDT10,MCDT10), I, J, KNT
