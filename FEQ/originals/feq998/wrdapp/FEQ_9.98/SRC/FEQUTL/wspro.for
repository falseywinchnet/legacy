...
C     + + + LOCAL VARIABLES + + +
     ...
     A     XARG(MXNSA), XE, XEPS, XINVAR, XS
      ...
      CHARACTER CVAL(MXNVAL)*64, HEADID*2, RECID*2, SECID*5, 
     A          TERM(MXNVAL)*1
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL BINSER, FDCNMN, GETVAL, INL80, INSPT
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETTOK, GETVAL, INL80, TABCHK, TWDOUT
...
C     + + + LOCAL VARIABLES + + +
     ...
     B     WSEL
      DOUBLE PRECISION DPVAL(MAXN)
      CHARACTER CVAL(MAXN)*64, HEAD*80, LABEL*50, LINE*80, LINE1*81,
     A          NAME*64, TERM(MAXN)*1, XSID*7
      LOGICAL THERE
 
      CHARACTER GETTOK*7, TABID*16
...
C                 make sure we get waht we want. 
 600              CONTINUE
                    READ(STD48,'(A80)') LINE
                    XSID = LINE(2:8)
C                          XSID = GETTOK(XSID)
...
C     + + + EXTERNAL NAMES + + +
      INTEGER LENSTR
      EXTERNAL CHKTAB, FNDELV, INL80, LKTK, LKTQC, LENSTR
...
C 61   FORMAT('J3                5  14    3')
 ...
 64   FORMAT('WS',8X,F12.2,F12.2,F12.2,F12.2,F12.2)
 65   FORMAT('WS',8X,F12.2,F12.2,F12.2,F12.2,F12.2,',')
