...
C     + + + EXTERNAL NAMES + + +
      INTEGER LENSTR
      EXTERNAL CHKCFC, CHKTAB, FRFCHN, INL80, SBFCHN, TABCHK, TWDOUT,
     A         CHANRAT_OPT, LENSTR, READ_TABID_PLUS, READ_TABID,
     B         STRIP_L_BLANKS, FNDELV
...
C     + + + OUTPUT FORMATS + + +
 ...
 74   FORMAT(/,' *ERR:629* ERRKND=',I5,' invalid. range 0 through 3.')
...
C     Set the local minimum flow.  Revise later when named-item input
C     is created.
      LOCAL_MINQ = MINQ
     ...
     I                   (STDOUT, LINE,
     O                    EFLAG, TABID, TAB, SAVEIT)

C      READ(LINE,1,ERR=991) TAB, SAVEIT
      WRITE(STDOUT,50) TABID(1:LENSTR(TABID)), SAVEIT
      WRITE(*,90) TABID(1:LENSTR(TABID))
     ...
     I          (STDIN, STDOUT,
     O           LINE)
      READ(LINE,27,ERR=991) CHAR4, TABTYP, CHRVEC
      WRITE(STDOUT,97) CHAR4, TABTYP, CHRVEC
      IF(TABTYP.EQ.5) THEN
        WRITE(STDOUT,71)
        TABTYP = 13
      ENDIF
      ...
      IF(CHRVEC(1).NE.'     ') THEN
        READ(CHRVEC(1),'(I5)') ERRKND
        IF(ERRKND.LT.0.OR.ERRKND.GT.3) THEN
...
C      READ(LINE,4,ERR=991) CHAR6, XSTAB
