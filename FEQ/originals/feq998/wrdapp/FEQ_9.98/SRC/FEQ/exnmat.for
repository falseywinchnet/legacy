...
C
      SUBROUTINE   ABREXP
     I                   (IPNT, GRAV, STDOUT, TIME, NEX, MREMC, EXNODT,
     I                    QE2, YE2, ZE,
     M                    EMC,
...
C     + + + DUMMY ARGUMENTS + + +
      ...
      REAL GRAV, PQL, PQR, PYL, PYR, QE2(NEX), RES, TIME, YE2(NEX),
     A     ZE(NEX)
...
C     + + + OUTPUT FORMATS + + +
 50   FORMAT(/,' *WRN:05* ABRUPT EXPANSION CANNOT ALLOW REVERSE',
     1       ' FLOW AT TIME=',F14.0,/,4X,'WITH FLOW=',1PE12.4,
...
C       REVERSE FLOW NOT PERMITTED
        ...
        WRITE(STDOUT,50) TIME, QE2(QNN), GETUSN(UNN), GETUSN(DNN)
...
C
      SUBROUTINE   CONTRL
     I                   (IPNT, STDOUT, NBN, NEX, OUTPUT, TIME, EXNODT,
...
C     + + + DUMMY ARGUMENTS + + +
      ...
      REAL DISCH, DXVEC(NBN), PDV, TIME, Y1(NBN), ZVEC(NBN)
...
C
      SUBROUTINE   EXCON
     I                  (IPNT, STDOUT, TIME, NEX, MLEMCV, EXNODT,
     I                   QE2, YE2, ZE,
     M                   EMC,
     O                   RES, PYL, PQL, PYR, PQR)
...
C     + + + DUMMY ARGUMENTS + + +
      ...
      REAL PQL, PQR, PYL, PYR, QE2(NEX), RES, TIME, YE2(NEX),
     A     ZE(NEX)
...
C       ERROR HERE
        WRITE(STDOUT,10) UNN, DNN, FSGN, TIME, YC, TC, AC, BC, DBC
        STOP 'Abnormal stop: errors found.'
 250  CONTINUE
...
C     + + + DUMMY ARGUMENTS + + +
      INTEGER IPNT, STDOUT, MLEMC, NEX
      INTEGER EMC(MLEMC)
      REAL PQL, PQR, PYL, PYR, QE2(NEX), RES, TIME, YE2(NEX), ZE(NEX)
...
C     
C
      SUBROUTINE DO_GATETABL(STDOUT, IPT, TIME,
     M                      EMC)
...
C         other.  Therefore, it sets its gate opening internally.
          CALL DO_GATETABL(STD6, IPT, TIME,
     M                      EMC)
...
C     + + + DUMMY ARGUMENTS + + +
      ...
      INTEGER EMC(MLEMC)
      REAL GRAV, PQL, PQR, PYL, PYR, QE2(NEX), RES, TIME, TWOG,
     A     YE2(NEX), ZE(NEX)
...
C             Traditional connection file.
              IF(ICDIN(I).EQ.1.OR.ICDIN(I).EQ.3) THEN
                READ(UNIN(I)) JTIN(LH), VALAIN(LH), VALBIN
              ELSE
                READ(UNIN(I)) JTIN(LH), VALAIN(LH)
              ENDIF
              LH = LH + 1
...
C
      SUBROUTINE   SETEXT
     I                (DT, WT, NBN, NBRA, NEX, BRPT, HSLOT, EPT, QEPS,
     M                    EMC, EXNODT,
...
C     + + + LOCAL VARIABLES + + +
     ...
     D     TEMP, TL, TOP, TP, TPTIME, TR, VL, VR, W, WC, WTC, YBAL,
...
C     FLOW IS A FUNCTION OF HEAD
 ...
 400  CONTINUE
        ...
        NSIGN = EMC(IPNT+3)
        CALL CONTRL
     I             (IPNT, STDOUT, NBN, NEX, OUTPUT, SNGL(TIME), EXNODT,
     I              EMC, Y1, ZVEC, DXVEC, EPT,
     O              PDV, DISCH)
        IEQ = IEQ + 1
        RES = QE2(DNN) -DISCH
...
C         EXPANSION/CONTRACTION WITH CRITICAL DEPTH POSSIBLE
          ...
          CALL EXCON
     I              (IPNT, STDOUT, SNGL(TIME), NEX, MREMC, EXNODT,
     I               QE2, YE2, ZE,
     M               EMC,
     O               RES, PYL, PQL, PYR, PQR)
          GOTO 599
...
C         BIDIRECTIONAL FLOW WITH TABLES AND SLOPE
 ...
 505    CONTINUE
          CALL ABREXP
     I               (IPNT, GRAV, STDOUT, SNGL(TIME), NEX, MREMC, 
     I                EXNODT, QE2, YE2, ZE,
     M                EMC,
     O                RES, PYL, PQL, PYR, PQR)
...
C         BRANCH ON THE SIGN OF THE NUMBER OF PATHS
          IF(EMC(IPNT+6).GT.0) THEN
            CALL TWOD6
     I                (IPNT, STDOUT, SNGL(TIME), NEX, EPT, EMC, QE2,
     I                 YE2, ZE,
     O                 RES, PYL, PQL, PYR, PQR)
          ELSE
            CALL TWOD14
...
C     A                                  ALPR*TR/AR)/GRAV
 ...
 1400 CONTINUE
        ...
        CALL SIDEWR
     I             (IPNT, STDOUT, SNGL(TIME), NEX, MREMC, EMC,
     I              EXNODT, QE2, YE2, ZE,
...
C
      SUBROUTINE   TWOD14
     I                   (IPNT, STDOUT, TIME, NEX, EPT, EMC, QE2,    
     I                    YE2, ZE,
     O                    RES, PYL, PQL, PYR, PQR)
...
C      WRITE(STDOUT,*) ' TWOD14: UNN=',UNN,' SYSGN=',SYSGN, ' FSGN=',FSGN
...
C      WRITE(STDOUT,*) ' TWOD14: HU=', HU, ' HD=',HD,' Q=',Q
...
C         FLOW NODE AT NOMINAL DOWNSTREAM NODE
          ...
          PQL = 0.0
        ENDIF
C      WRITE(STDOUT,*) ' TWOD14 SPECIAL CASE: RES=',RES
...
C      WRITE(STDOUT,*) ' TWOD14: RES=',RES,' HUTAB=',HUTAB
C      WRITE(STDOUT,*) ' PYL=',PYL,' PQL=',PQL,' PYR=',PYR,' PQR=',PQR
...
C
      SUBROUTINE   TWOD6
     I                  (IPNT, STDOUT, TIME, NEX, EPT, EMC, QE2, YE2,
     I                   ZE,
     O                   RES, PYL, PQL, PYR, PQR)
...
C     + + + LOCAL VARIABLES + + +
     ...
     A        TYPE, UDTAB, UNN, ZTAB
      REAL DQED, DQEU, EL, ER, HBASE, PDV, Q, RDUM, YL, YR
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETTYP, LKTAB, TDLK13, TDLK6
C***********************************************************************
C     GET KEY VALUES
...
C         FLOW FROM NOMINAL UPSTREAM NODE TO DOWNSTREAM NODE
          ...
          IF(TYPE.EQ.6) THEN
            CALL TDLK6
     I                (STDOUT, UDTAB, 6, MFTAB, TIME, ER, EL, HBASE,
     O                 Q, DQED, DQEU, FREE)
          ELSE
            CALL TDLK13
     I                 (STDOUT, UDTAB, 13, MFTAB, TIME, ER, EL, HBASE,
     O                  Q, DQED, DQEU, FREE)
          ENDIF
          IF(SYSGN.GT.0) THEN
            RES = RES - Q
...
C         FLOW FROM NOMINAL DOWNSTREAM NODE TO UPSTREAM NODE
          ...
          IF(TYPE.EQ.6) THEN
            CALL TDLK6
     I                (STDOUT, DUTAB, 6, MFTAB, TIME, EL, ER, HBASE,
     O                 Q, DQED, DQEU, FREE)
          ELSE
            CALL TDLK13
     I                 (STDOUT, DUTAB, 13, MFTAB, TIME, EL, ER, HBASE,
     O                  Q, DQED, DQEU, FREE)
          ENDIF
          IF(SYSGN.GT.0) THEN
            RES = RES + Q
...
C     + + + DUMMY ARGUMENTS + + +
      INTEGER IPNT, MLEMC, NEX, STDOUT
      INTEGER EMC(MLEMC)
      REAL PQL, PQR, PYL, PYR, QE2(NEX), QE1(NEX), RES, TIME, YE2(NEX),
     A     ZE(NEX)
...
C       FLOW FROM NOMINAL UPSTREAM NODE TO DOWNSTREAM NODE
        ...
        CALL TDLK15
     I             (STDOUT, UDTAB, MFTAB, TIME, ER, EL, HG, HBASE,
...
C       FLOW FROM NOMINAL DOWNSTREAM NODE TO UPSTREAM NODE
        ...
        CALL TDLK15
     I             (STDOUT, DUTAB, MFTAB, TIME, EL, ER, HG, HBASE,
     O              Q, DQED, DQEU, NEWHG, FTYPE, FTYPE_CODE)
        Q = GATE_FACTOR*Q
        DQEU = GATE_FACTOR*DQEU
        DQED = GATE_FACTOR*DQED
