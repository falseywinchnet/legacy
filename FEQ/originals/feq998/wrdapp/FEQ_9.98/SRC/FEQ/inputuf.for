...
C       sense for this tributary-area unit.
        WRITE(STDOUT,*) ' IRRIG_CHK: GAGE=',GAGE,
     A        ' GSTART=',GSTART(GAGE),' GEND=',GEND(GAGE)

        WRITE(STDOUT,*) ' IRRIG_LUI_APPLY=',IRRIG_LUI_APPLY(I)
        WRITE(STDOUT,*) ' IRRIG_LUI_COMPUTE=',IRRIG_LUI_COMPUTE(I)
...
C     Local
     ...
     A          TABID*16
      ...
      EXTERNAL INL80, STRIP_L_BLANKS, GET_INTERNAL_TAB_NUMBER
C     **************************FORMATS*********************************
50    FORMAT(' ',A80)
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL FNDSTA, GETTOK, INL120, INL120B, INSNOD, KIL,
...
C       on the initial element.
     ...
     A       XVEC(J), ZVEC(J), KA(J), KD(J), HLTABID(1:HLIDLEN),
...
C     + + + LOCAL VARIABLES + + +
     ...
     A        MLOC, MODE, N2EQ, NGTMP, NT, USBR, STATUS
      INTEGER N(10)
      REAL F(5), RDUM, HDATUM
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL CNVRTN, GETTOK, INL80, KIL, KILC, LSATAB, POP_LINE,
     A         LENSTR, FIND_TABIDL, INL196, FIND_HDD_FROM_TABN
...
C     + + + OUTPUT FORMATS + + +
 ...
 34   FORMAT(4X,'Forced value at boundary: ',4X,/,
     1'  Type=',A5,'   Ups node=',A5,' Dir=',A5,
     2' Time-series Id= ',A,'  Flow=',F10.2,/,4X,' Factor=',F5.2)
 ...
 43   FORMAT(4X,' Q vs. Head table Id=',A,' Controlling level vs.',
     1     ' Time source=',S,/,4X,'Limiting Q vs. Controlling level',
 ...
 56   FORMAT(4X,'TabId for pos flow=',A,' TabId for neg flow=',A,
     2 /,4X,'TabId for adjustment factor=',A,' Base elev.=',A10,
 ...
 60   FORMAT(4X,'Match average of elevations at nodes:',2A5,' with',
...
C     I            (IN, STDOUT,
C     O             LINE)
C        READ(LINE,24,ERR=991) CODE, NC, F
C        DO 1001 I=1,10
C          NC_LONG(I) = NC(I)
...
C            TABLE
             ...
             WRITE(STDOUT,43) NC_LONG(5), NC_LONG(6), NC_LONG(7), F(1)
...
C            ELEVATION FOR DEFINING DEPTH AT THE CRITICAL SECTION.
             ...
             WRITE(STDOUT,41) N(5), NC_LONG(6), F(1), F(2), F(3)
...
C           model will not run because a table is missing.
     ...
     A                     NC_LONG(6), NC_LONG(7), F(1), CHR10, F(3)
        ENDIF
        IF(F(1).LT.0.0.OR.F(1).GT.1.0) THEN
          WRITE(STDOUT,62) F(1)
          EFLAG = 1
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     A          CD5T6_TAB_DATUM*4, FNAME2*64, FNAME1*64
...
C     + + + LOCAL VARIABLES + + +
     ...
     B     NEW_GENSCN_FEO*4, GHOME*64
 
C     + + + EXTERNAL NAMES + + +
      INTEGER GET_UNIT
      EXTERNAL INFO1, INFO2, INL80, KIL, FREE_UNIT,
...
C     Clear table id counter in common tabid.cmn
     ...
     I          (IN, FNAME2, FNAME1,
     O           STDOUT, 
     O           EFLAG, SSEPS, GRAV, VUNIT, ABSTOL, OUTPUT, PROUT, TIME,
...
C
     ...
     O                   PROUT, TIME, LDCALL, ZIY, NODEID, MINPRT,
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     A        NUMLIM, OUTPUT, PROUT, START_EQ, END_EQ
      INTEGER GEQVEC(MNBRA)
      REAL ABSTOL, GRAV, SSEPS, VUNIT, ZIY, DTMIN_OUT
      REAL*8 TIME
...
C     + + + LOCAL VARIABLES + + +
      ...
      REAL EPSFAC
      REAL*8 HR
      CHARACTER  GENAME*5, NAME*4, GLB_HOME*64,
     A          LINE120*120, DATE*8, ZEIT*10, ZONE*5,
     B          LONGLINE*196, TEMP_FILE_NAME*128,
     C          SB_HOME*64, WORK*128
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL CHKDAT, CHKMJD, INL80, INL120B, KIL, MJD,
     A         INL196_ALL, STRIP_L_BLANKS, GET_UNIT,
     B      WHAT_EXECUTABLE
...
C     + + + OUTPUT FORMATS + + +
 ...
 27   FORMAT(/,' Print out will be given every ',I5,' time steps')
...
80    FORMAT(' Version: ',F6.2,' Version date: ',A17,10X,
...
C         last line of the title block. 
     ...
     A          DATE(5:6), DATE(7:8), ZEIT(1:2), ZEIT(3:4), ZEIT(5:10)
        ...
        ENDIF
100   CONTINUE
C     If we get here, the user has  used more than 50 lines for the header or
...
105   CONTINUE      
     ...
     O             START_EQ, END_EQ, DTMIN_OUT, GLB_HOME)
...
C      IF(POINT.EQ.YES) LDCALL = 1
     ...
     O            EFLAG)
      ...
      SJTIME = DBLE(MJD(SYR,SMN,SDY)) + SFRAC/24.D0
      UJTIME = SJTIME
      JTIME = SJTIME
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     A        NFREE, NRWTA, OUTPUT, QPN, RESNUM
      INTEGER  RTAP(MNFREE), RWTA(MNFREE)
      REAL GRAV, HSLOT, RTAFAC(MNFREE), VUNIT
...
C     + + + LOCAL VARIABLES + + +
     ...
     B        SPOUT_STYLE, TABLE, TWON, DEFKNT
      INTEGER CHKSQ(MNETRY), BASE_NODE_VEC(MNEX), WORK(MLPDA)
      
      REAL AIRSPW, WATSPW
      CHARACTER CHR5*5, CHR6*6, HEAD*80, LINE*80, NXTNAM*128,
     A          C5VEC(MNEX)*5, KEY*16
      LOGICAL THERE
...
C     + + + EXTERNAL NAMES + + +
     ...
     A         EXINIT, FTABIN, GETINN, GETUSN, INL80, KIL, MAKEMC,
     B         MAKJUN, MAKMAT, NZDCON, OPIN, OUTIN, PFIN, PMPCTB, RDFIN,
     ...
     H         OUTPUT_SCHEMATIC_SCRIPT, STAT_TRAN
...
C     + + + OUTPUT FORMATS + + +
 ...
 59   FORMAT(' ','Number of branches:',I5,/
     ...
     C     ,' Number of operation blocks:',I5,/
     D     ,' Function table number:',I5,/
     E     ,' Length of the function table:',I7,/
     G     ,' Number of point flows:',I5,/
     H     ,' Length of Network-Matrix Control:',I7,/
     I     ,' Number of rows in array of partial deriv.:',I5,/
     J     ,' Length of array of partial deriv. :',I7,/
...
88    FORMAT(/,'*ERR:408* FEQ has found ',I8,' function tables',
     ...
     D ' missing.')
C***********************************************************************
C     INPUT THE BRANCH DESCRIPTION TABLES
...
C     CLEAR THE EXTERIOR NODE COUNTER FOR INTERNAL EXTERIOR NODE NUMBERS
 ...
 203    CONTINUE
     ...
     I            GISID_TO_NODEID, TABID_TO_NODEID,
     M            EFLAG, USBRIN, ENODEN, NBRA, 
...
C         The user has requested irrigation computations.  
          CALL IRRIG_INPUT(IN, STDOUT, NEX,
     M                     ENODEN, EFLAG)
        ELSE
          BACKSPACE(IN)
          IRRIG_KNT = 0
...
C     INPUT THE POINT FLOW INFORMATION
     ...
     O           LINE)
      ...
      WRITE(STDOUT,12) HEAD
      CALL PFIN
     I         (IN, STDOUT, MCPFPT, NBRA, BRPT,
     O          EFLAG, LPNT, MF, PFPNT)
...
C         The default function table has been input.
      ...
      IF(FTKNT+DEFKNT.NE.NUM_TABID) THEN
        WRITE(STDOUT,88) FTKNT, NUM_TABID
...
C       output.
     ...
     M                                            EFLAG)
        ...
        IF(POINT.EQ.NO) GOTO 2100

          CALL CHKPT
     I             (STDOUT, LPNT, MFTNUM, FTPNT,
     M              PFPNT,
     O              EFLAG)
...
C     Adjust the arguments for reservoir storage tables.
C      CALL RTBADJ
C     I           (STDOUT, NEX, EXNODT, MREMC, EMC,
C     M            ZE, YE1)
...
C       Transfer station values for the exterior nodes on branches
C       so that they follow the same pattern as the other exterior
C       nodes.
        CALL STAT_TRAN(NEX, 
     M                    EXNODT)
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETINB, GETNXT, GETUSB, INL80
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETINB, GETINN, GETTOK, INL80, KIL, STRIP_L_BLANKS,
...
C     Set the elapsed time since last change in setting to a large
C     value to ensure that a change takes place
      FTAB(FTP+5) = 1.E30
...
C     + + + LOCAL VARIABLES + + +
     ...
     B        ITEM_END(MAXN)
      CHARACTER  HEAD*120, HEAD1*7, HEAD2*7, LINE*120, NAME*16,
     A          OUTNAM*64, CHAR80*80, TABID*16, JUST*5, CHAR5*5
 
C     + + + INTRINSICS + + +
      INTRINSIC ABS
 
C     + + + EXTERNAL FUNCTIONS + + +
C      CHARACTER GETTOK*5
 
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETINB, GETINN, GETTOK, INL120, KIL, LSTAB, FREE_UNIT,
     A         GET_INTERNAL_TAB_NUMBER, STRIP_L_BLANKS,
     B         GET_ITEM_LIMITS, READ_SPOUT_ITEMS
...
C          STOP 'Abnormal stop: errors found.'
     ...
     O             LINE)
      ...
      READ(LINE,1,ERR=991) CHAR80
      CALL PROCESS_OLD_IOUNITS(STDOUT, CHAR80,
     O                               UNIT, OUTNAM)
      CALL MAYBE_ADD_HOME(
     M                    OUTNAM)
      WRITE(STDOUT,2) OUTNAM
      IF(UNIT.NE.0) THEN
        OPEN(UNIT=UNIT, FILE=OUTNAM, STATUS='UNKNOWN',IOSTAT=IOFLAG)
        IF(IOFLAG.NE.0) THEN
          IT = LEN_TRIM(OUTNAM)
...
C     WRITE CURRENT VALUE OF PAGESP TO THE FIRST LINE OF THE FILE
      IF(NOUT.GT.0)  WRITE(UNIT,50) PAGESP, NOUT, UJTIME, VJTIME,
     A                              LINE_KNT
...
C
      SUBROUTINE   PFIN
     I                 (IN, STDOUT, MLPNT, NBRA, BRPT,
     O                  EFLAG, LPNT, MF, PFPNT)
 
C     + + + PURPOSE + + +
C     Input the point load information.
 
      IMPLICIT NONE
C     + + + DUMMY ARGUMENTS + + +
      INTEGER EFLAG, IN, STDOUT, LPNT, MLPNT, NBRA
      INTEGER BRPT(8,NBRA), PFPNT(3,MLPNT)
      REAL MF(MLPNT)
...
C     + + + LOCAL VARIABLES + + +
      INTEGER BRA, BRAIN, NODE, TABLE, TYPE
      REAL MFAC
      CHARACTER LINE*80, NODEIN*5, TABID*16
 
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETINB, GETINN, INL80, KIL, KILC,
     A         STRIP_L_BLANKS, GET_INTERNAL_TAB_NUMBER
 
C     + + + INPUT FORMATS + + +
 35   FORMAT(I5,A5,I5,A16,F10.0)
 
C     + + + OUTPUT FORMATS + + +
 36   FORMAT(' ',I5,A5,I5,1X,A16,F10.5)
C***********************************************************************
      LPNT = 1
 1000 CONTINUE
         CALL INL80
     I             (IN, STDOUT,
     O              LINE)
         READ(LINE,35,ERR=991) BRAIN, NODEIN, TYPE, TABID, MFAC
         IF(BRAIN.EQ.-1) GOTO 2000
         WRITE(STDOUT,36) BRAIN, NODEIN, TYPE, TABID, MFAC
         CALL GETINB
     M              (BRAIN,
     O               EFLAG, BRA)
          CALL GETINN
     ...
     O                                 TABLE)
         ...
         IF(NODE.LT.BRPT(1,BRA).OR.NODE.GT.BRPT(2,BRA))
     1      CALL KILC
     I                (NODEIN, -2,
     M                 EFLAG)
         IF(TYPE.LE.0.OR.TYPE.GT.2) CALL KIL
     I                                       (18,
     M                                        TYPE, EFLAG)
         IF(NODE.EQ.BRPT(1,BRA)) CALL KILC
     I                                     (NODEIN, -28,
     M                                      EFLAG)
 
C     TABLE IDS WILL BE CHECKED ELSEWHERE
 
         IF(LPNT.GE.MLPNT) CALL KIL
     I                              (19,
     M                               LPNT, EFLAG)
         PFPNT(1,LPNT) = TYPE
         PFPNT(2,LPNT) = TABLE
...
C     COMPUTE THE NODE LOCATION IN THE BRANCH VECTORS
 
         PFPNT(3,LPNT) = BRPT(3,BRA)-BRPT(1,BRA)+NODE
         LPNT = LPNT+1
         GOTO 1000
 2000 CONTINUE
      PFPNT(1,LPNT) = -1
...
C     + + + LOCAL VARIABLES + + +
     ...
     A        TERM_CLASS(MAX_KNT), UN
      ...
      CHARACTER HEAD*80, LINE*112, NAME*80, 
     A          ITEMS(MAX_KNT)*64, TS_ID_FOR_FILE*16,
     B          C_FACTOR*10, WORK*196

      REAL FACTOR

C     + + + EXTERNAL NAMES + + +
      INTEGER GET_UNIT
      EXTERNAL INL112, KIL, GET_UNIT
...
C       Perhaps add home directory to the file names.
        CALL MAYBE_ADD_HOME(
     M                      TSFNAM)
        CALL MAYBE_ADD_HOME(
     M                      FFFNAM)
...
C     Create parameters for coding ACTION
     ...
     A        NITEM, ITEM_START(MAXN), ITEM_END(MAXN), IT, NXT
      CHARACTER HEAD*120, ITEM*4, LINE*120, NAME*80, TYPE*4, USNODE*6,
     A          JUST*5, ACTION*4
 
C     + + + EXTERNAL NAMES + + +
      INTEGER GET_UNIT, LENSTR

      EXTERNAL GETINB, GETINN, INL120, KIL, GET_UNIT, LENSTR,
     A         GET_ITEM_LIMITS
...
C         Get the next line.
     ...
     O                         ACTION, USBRA, USNODE, ITEM, TYPE, NAME)
        ...
        IF(ACTION(1:2).NE.'-1') THEN
          ...
          IF(HECDSS.EQ.0) THEN
C           Assign an I/O unit but only if action is OUT in first three characters or blank
...
C     will be returned.
      ...
      CHARACTER UNIT_STRING*80, NAME_STRING*64
