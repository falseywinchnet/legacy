...
C
C
      SUBROUTINE SET_INITIAL_OPER_BLK(NBLK, OPBLK, TIME, EPT, 
     M                                EMC)
...
C
     ...
     I                    OUTPUT, TIME, SFAC, BRPT, EMC, QEPS, EPT,
     M                    EXNODT,
     O                    EFLAG)
...
C     + + + EXTERNAL NAMES + + +
     ...
     A         INL80, LKTAB, SFPSBM, XLKT23, GETUSB, GETUSN
...
C     READ TITLE CARD
     ...
     I          (IN, STDOUT,
     O           LINE)
      READ(LINE,1000) (CHAR(I), I=1,20)
      WRITE(STDOUT,2000) (CHAR(I), I=1,20)
...
C         NBR = GETINB(NBR, EFLAG)
 ...
 100  CONTINUE
     ...
     O           LINE)
      READ(LINE,1000) (CHAR(I), I=1,20)
      WRITE(STDOUT,2001) (CHAR(I), I=1,20)
...
C               is a Code 4 specified at the node.
                ...
                ELSE
                  YR = YE1(SNODE)
                  CALL CD4QY
     I                      (STDOUT, NB, SNODE, NBRA, NEX, NBN, TIME,
...
C             Code 4 types 1, 2, or 3.
              ...
              ELSE
                CALL CD4QY
     I                    (STDOUT, NB, DEXN, NBRA, NEX, NBN, TIME, BRPT,
...
C                 Find the entry in EMC.
     ...
     O                      IPNT)
                  ...
                  YR = YE1(SNODE)
                  CALL CD56QY
     I                   (STDOUT, IPNT, NEX, TIME, EMC, MAXIT, YR, EPSB,
     I                    QEPS, EPT,
     M                    YL)
...
C     to a larger negative value at start.  
      DO 950 NBR=1,NBRA
        FN = BRPT(3,NBR)
        LN = BRPT(4,NBR)
        DO 945 J=FN,LN
...
C     + + + LOCAL VARIABLES + + +
      INTEGER DYT, I, J, MNT, YRT
      REAL RUNOFF(MNDIFA)
      REAL*8 DUMMY, DYF, RI(MNDIFA), TSFDT
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL IWYR, LPYEAR
 
C     + + + OUTPUT FORMATS + + +
 50   FORMAT(/,' *BUG:04* INVALID JTIME FOR TSF RECORD JTIME = ',
     1     1PD25.16)
 52   FORMAT(' *BUG:36 UNEXPECTED END OF FILE IN DTSF IN BFINIT.',/,
...
C     HAVE BEEN READ PREVIOUSLY
 ...
 90   CONTINUE
        ...
        IF(TSFTYP.EQ.0) THEN
          READ(TSFDSN, END=5000) BJTIME(1),
     A                                SFRAC, SYR, SMN, SDY, NDFVAL,
     B                                (TSFBUF(1,J),J=1,NDFVAL)
        ELSE
          READ(TSFDSN, END=5000) BJTIME(1), SFRAC, SYR, SMN, SDY,
...
C         SKIP THIS EVENT
 110      CONTINUE
            IF(TSFTYP.EQ.0) THEN
              READ(TSFDSN) BJTIME(1), DYF, YRT, MNT, DYT, NDFVAL,
     1                    (TSFBUF(1,J),J=1,NDFVAL)
            ELSE
              READ(TSFDSN) BJTIME(1), DYF, YRT, MNT, DYT,
     1                     (RUNOFF(J), J=1,NDFVAL)
            ENDIF
...
C     FILL THE REMAINDER OF THE BUFFER SET
      ...
      DO 200 I=2,NBUFF
        ...
        IF(TSFTYP.EQ.0) THEN
          READ(TSFDSN, END=5000) BJTIME(I), DYF, YRT, MNT, DYT, NDFVAL,
     1                 (RI(J),J=1,NDFVAL)
        ELSE
          READ(TSFDSN, END=5000) BJTIME(I), DYF, YRT, MNT, DYT,
     1                 (RUNOFF(J), J=1,NDFVAL)
          DO 106 J=1,NDFVAL
            RI(J) = RUNOFF(J)
 106      CONTINUE
...
C     Get the state of the system from the dataset given by BWFDSN.
      ...
      IMPLICIT NONE
C     + + + DUMMY ARGUMENTS + + +
      INTEGER BWFDSN, MBLK, NBLK, NBN, NBRA, NEX
      INTEGER OPBLK(MBLK)
      REAL WT
...
C***********************************************************************
 ...
 100  CONTINUE
      ...
      DO 200 I=1,NEX
        READ(BWFDSN) QE1(I), YE1(I), AE1(I), MYE(I), MQE(I)
 200  CONTINUE
...
C
      SUBROUTINE   CD4QY
     I                  (STDOUT, NB, HDNODE, NBRA, NEX, NBN, TIME, BRPT,
     I                   EXNODT, EMC, MAXIT, EPSB, OUTPUT, EPT,
     M                   YR)
...
C       FIND DISCHARGE AT THE INPUT ELEVATION.
        ...
        CALL CONTRL
     I             (IPNT, STDOUT, NBN, NEX, OUTPUT, TIME, EXNODT, EMC,
     I              Y1, ZVEC, DXVEC, EPT,
     O              PDV, DISCH)
        IF(PDV.EQ.0.0) THEN
...
C      WRITE(STDOUT,*) ' ENTERING CD56QY: YL=',YL,' YR=',YR
      ...
      IF(FLOW.EQ.0.0) THEN
        ...
        STOP 'Abnormal stop: errors found.'
      ENDIF
C      WRITE(STDOUT,*) ' NPATH=',NPATH,' FLOW=',FLOW
      IF(FLOW.LT.0.0) THEN
        FSGN = -SYSGN
      ELSE
        FSGN = SYSGN
...
C         than downstream.
 ...
 100    CONTINUE
          CALL TWOD6
     I              (IPNT, STDOUT, TIME, NEX, EPT, EMC, QE2, YE2, ZE,
...
C     + + + EXTERNAL NAMES + + +
      EXTERNAL GETINN, GETUSB, GETUSN, INL80, READ_FREE_NODE_ITEMS,
...
C     + + + LOCAL VARIABLES + + +
      INTEGER FBNODE, FLAG, FNBN, FNBRA, FNEX
      REAL*8 FJTIME
      LOGICAL THERE
...
C***********************************************************************
      ...
      ELSE
        ...
        ELSE
          WRITE(STDOUT,*) ' FILE NAMED:', NAME,' NOT FOUND.'
...
C         FILL THE CURRENT BUFFER FROM THE FILE                               
          ...
          IF(DSS_INDEX_IN(I).EQ.0) THEN                                       
            DO 150 J=MINROW(I),MAXROW(I)                                      
              IF(ICDIN(I).EQ.1.OR.ICDIN(I).EQ.3) THEN                         
                READ(UN) JT, VALAIN(J), VALBIN                                
              ELSE                                                            
                READ(UN) JT, VALAIN(J)                                        
              ENDIF                                                           
...
C     + + + LOCAL PARAMETERS + + +
      INTEGER ID
      PARAMETER(ID = 8+2*MNDIFA)
 
C     + + + LOCAL VARIABLES + + +
      INTEGER I, J, LIMIT, IOSTATUS
      INTEGER A(ID), TEMP(10)
...
C     THE DTSF
      ...
      READ(TSFDSN,IOSTAT=IOSTATUS) (A(I), I=1,7)
      IF(IOSTATUS.NE.0) THEN
        WRITE(STD6,*) ' Problem in reading DTSF in READOC'
        WRITE(STD6,*) ' IO Status value=',IOSTATUS
        STOP 'Abnormal stop: errors found.'
      ENDIF
...
C     REOPEN THE TSF FILE TO RESTART AT FIRST RECORD
      ...
      IF(TSFNAM.EQ.' ') THEN
        OPEN(TSFDSN, FILE='DTSF', FORM='UNFORMATTED', STATUS='OLD')
      ELSE
 ...
 13   CONTINUE
      ...
      READ(TSFDSN) (A(I), I=1,LIMIT)
      WRITE(STD6,14) (A(I), I=1,MIN(32,LIMIT))
      WRITE(STD6,10) (TEMP(I), I=1,4), NDFVAL
      WRITE(STD6,11) (TEMP(I), I=5,10)
 
      DO 15 I=1,NDFVAL
        READ(TSFDSN) (A(J), J=1,LIMIT)
        WRITE(STD6,16) I, (A(J), J=1,MIN(20,LIMIT))
 15   CONTINUE
...
C
     ...
     I                   EPSB, GRAV, IN, MAXIT, NBN, OUTPUT, TIME, SFAC,
     I                   NBLK, OPBLK, BWFDSN, FFFDSN, DIFFUS, UNDERF,
     I                   IFRZ, DT, GETDSN, UJTIME,
     I                   EPT, BNODE, QEPS,
     M                   EMC,
     O                   WT, SITER)
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     A        STDOUT, MAXIT, NBLK, NBN, NBRA, NEX, OUTPUT, TSFDSN
      INTEGER BRPT(8,NBRA), EMC(EPT), EXNODT(9,NEX),
     A        OPBLK(MNBLK)
      REAL EPSB, GRAV, QEPS, SFAC, SITER, TIME, WT
      REAL*8 DT, UJTIME
...
C     Close the user input file
      ...
      IF(NBLK.GT.0) THEN
        ...
        CALL SET_INITIAL_OPER_BLK(NBLK, OPBLK, TIME, EPT,
     M                            EMC)
      ENDIF 
      IF(DIFFUS.EQ.YES) THEN
        ...
        ENDIF
        IF(BWFNAM.EQ.' ') THEN
          OPEN(BWFDSN, FILE='BWFD', FORM='UNFORMATTED', STATUS='NEW')
        ELSE
...
C         We can adjust this table. 
 ...
 300      CONTINUE
C         Change the address for last acces to the 2nd level
          ITAB(ADRS+3) = LA + XOFF
