...
C
     ...
     I                    BNODE, QCHOP, BNI, BNQ, TAI, TAQ, TAQBN, 
     I                    WSI, WSQ, OLD_SUMMARY,
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     A        RESVEC(NEX)
      REAL DTVEC(MNFRDT), HSLOT, QCHOP, SFAC, VUNIT, WT
      REAL*8 D1, DT, S1, BNI, BNQ, TAI, TAQ, TAQBN, WSI, WSQ
...
C             FROZEN TIME OVER. SAVE STATE OF SYSTEM AS NEEDED.
     ...
     I                (BWFDSN, WT, NBRA, NBN, NEX, NBLK, OPBLK)
                WRITE(STD6,*) ' BWF BEING WRITTEN AT IFRZ=0'
              ENDIF
...
C         OUTPUT RESULT AND MAX ELEVATION SUMMARY
     ...
     I                SFAC, BRPT, EXNODT, QCHOP)
          ...
          IF(OLD_SUMMARY.NE.'YES') THEN
            CALL ZSUMRY
     I                 (STDOUT, NBRA, NODEID, SNGL(TIME), SFAC, EMC,     
...
C         Close time series output files.
          IF(NFOUT.GT.0) CALL WRQUIT(STDOUT)
...
C     + + + DUMMY ARGUMENTS + + +
      ...
      REAL HSLOT, QCHOP, SFAC, SITER, VUNIT, WT
      REAL*8 DT
...
C     + + + OUTPUT FORMATS + + +
 ...
 3022 FORMAT(' Event computation time=',F10.2,' secs or ',F10.4,' mins')
...
C          CALL TIMINC(-DT, JTIME, YR, MN, DY, DYFRAC, LEAP)
          ...
          IF(FFFDSN.GT.0) THEN
            ...
            IF(OLD_SUMMARY.NE.'YES') THEN
              CALL ZSUMRY
     I                   (FFFDSN, NBRA, NODEID, SNGL(TIME), SFAC, EMC,
     I                    BRPT, EXNODT, NEX, HSLOT, VUNIT, QCHOP)
            ELSE
              CALL ZSUMRY_OLD
     I                   (FFFDSN, NBRA, NODEID, SNGL(TIME), SFAC, EMC,
