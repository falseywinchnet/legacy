...
int main(int argc, char **argv) {
  try {
    ...
    for (int part = 1; part <= 6; ++part)
      require(p.rows("WHAFIS PART " + std::to_string(part), "1").size() ==
                  std::size_t(expected_counts[part - 1]),
              "WHAFIS report table lost records.");
    require(
        numeric(p.rows("WHAFIS PART 2", "1").front(), "STATION") == -22.6f &&
    ...
    require(textual(zones.at(9), "ZONE") == "X" &&
                numeric(zones.at(9), "FHF") == 55,
            "WHAFIS unprinted high-ground interval did not become the original "
            "X zone.");
    require(p.rows("RUNUP ZONE", "1") ==
                Project::deserialize(before).rows("RUNUP ZONE", "1"),
